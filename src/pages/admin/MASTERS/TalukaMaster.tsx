...
// Sample taluka data for demonstration
...
// Taluka Management Dashboard
const TalukaDashboard = () => {
  const [showTalukaForm, setShowTalukaForm] = useState(false);
  ...
  const [viewingTaluka, setViewingTaluka] = useState(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [activeFilter, setActiveFilter] = useState('all'); // 'all', 'active', or 'inactive'

  const toggleTalukaForm = () => {
    setShowTalukaForm(!showTalukaForm);
    if (!showTalukaForm) {
      setEditingTaluka(null); // Reset editing taluka when opening form for new taluka
    }
  };

  const handleEdit = (taluka) => {
    setEditingTaluka(taluka);
    setShowTalukaForm(true);
  };

  const handleView = (taluka) => {
    setViewingTaluka(taluka);
  };

  const handleSaveTaluka = (talukaData) => {
    if (editingTaluka) {
      // Update existing taluka
      setTalukas(talukas.map(taluka => 
        taluka.id === editingTaluka.id ? { ...talukaData, id: taluka.id } : taluka
      ));
    } else {
      // Add new taluka
      const newTaluka = {
        ...talukaData,
        id: talukas.length > 0 ? Math.max(...talukas.map(t => t.id)) + 1 : 1
      };
      setTalukas([...talukas, newTaluka]);
    }

    setShowTalukaForm(false);
  ...
  // Filter talukas based on search term and active filter
  const filteredTalukas = talukas.filter(taluka => {
    // First apply status filter
    if (activeFilter === 'active' && !taluka.isActive) return false;
    if (activeFilter === 'inactive' && taluka.isActive) return false;
    
    // Get district name for search
    const districtName = getDistrictName(taluka.districtId);
    
    // Then apply search term filter
    return (
      taluka.shortName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      taluka.longName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      taluka.printName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      taluka.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="container mx-auto py-6 px-4">
      {/* Header section with search bar and Add Taluka button */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold mb-4">Taluka Management</h1>
      ...
      <div>
        {showTalukaForm ? (
          <TalukaMasterForm 
            onClose={toggleTalukaForm} 
            onSave={handleSaveTaluka} 
            initialData={editingTaluka}
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredTalukas.length > 0 ? (
                    filteredTalukas.map((taluka) => (
                      <tr 
                        key={taluka.id} 
                        className={taluka.isActive ? "bg-[#D1FFBD]" : "bg-[#f3c2c2]"}
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          {getDistrictName(taluka.districtId)}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          {taluka.shortName}
                        </td>
                    ...
                    <tr>
                      <td colSpan={7} className="px-6 py-4 text-center text-sm text-gray-500">
                        {searchTerm 
                          ? `No talukas found matching "${searchTerm}"${activeFilter !== 'all' ? ` with '${activeFilter}' status` : ''}`
                          : `No ${activeFilter !== 'all' ? activeFilter : ''} talukas found`
                        }
                      </td>
                    </tr>
                  )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full">
            <div className="flex items-center justify-between p-4 bg-orange-500 text-white rounded-t-lg">
              <h2 className="text-xl font-bold">Taluka Details</h2>
            ...
            <div className="p-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-900">{getDistrictName(viewingTaluka.districtId)}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-orange-500">Short Name</p>
                  <p className="mt-1 text-sm text-gray-900">{viewingTaluka.shortName}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-orange-500">Long Name</p>
                  <p className="mt-1 text-sm text-gray-900">{viewingTaluka.longName}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-orange-500">Print Name</p>
                  <p className="mt-1 text-sm text-gray-900">{viewingTaluka.printName}</p>
                </div>
                <div className="col-span-2">
                  <p className="text-sm font-medium text-orange-500">Description</p>
                  <p className="mt-1 text-sm text-gray-900 max-h-16 overflow-y-auto">{viewingTaluka.description}</p>
                </div>
              </div>
...
// Taluka Master Form component with edit capabilities and district dependency
const TalukaMasterForm = ({ onClose, onSave, initialData, districts }) => {
    ...
    <div className="max-w-4xl mx-auto my-8 bg-white rounded-lg shadow-md">
      ...
      <div className="flex items-center p-4 bg-orange-500 text-white rounded-t-lg">
        ...
        <h1 className="text-2xl font-bold flex-grow text-center">
          {initialData ? 'UPDATE TALUKA' : 'ADD TALUKA'}
        </h1>
        <button 
          onClick={onClose}
      ...
      <div className="border-2 border-blue-500 rounded-lg m-6 p-6">
        {/* Taluka Details Section */}
        <div className="relative mb-6 bg-white rounded-lg">
          <h2 className="text-lg font-semibold mb-6 px-4 py-2 bg-orange-500 text-white rounded">TALUKA DETAILS</h2>
          ...
          <div className="mb-6">
            ...
            <p className={`mt-1 text-sm ${formData.isActive ? "text-green-600" : "text-red-600"}`}>
              {formData.isActive ? "Y - Taluka is active" : "N - Taluka is inactive"}
            </p>
          </div>
          
          {/* District selection (full row) */}
          <div className="mb-6">
            <label className="block text-sm font-medium text-black-500 mb-2">District<span className="text-red-500">*</span></label>
            <select
              name="districtId"
