...
// Sample DeathCause data for demonstration
const initialDeathCauses = [
  {
    id: 1,
    isActive: true,
    shortName: 'HTA',
    longName: 'Heart Attack',
    printName: 'Heart Attack',
    description: 'Sudden loss of blood flow to part of the heart muscle'
  },
  {
    id: 2,
    isActive: true,
  ...
  },
  {
    ...
    longName: 'Cancer',
    printName: 'Cancer',
    description: 'Uncontrolled growth and spread of abnormal cells in the body'
  }
]

// DeathCauses Management Dashboard for Zila Parishad / Gramin Vibhag
const Dashboard = () => {
  const [showDeathCauseForm, setShowDeathCauseForm] = useState(false);
  const [DeathCauses, setDeathCauses] = useState(initialDeathCauses);
  const [editingDeathCause, setEditingDeathCause] = useState(null);
  const [viewingDeathCause, setViewingDeathCause] = useState(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [activeFilter, setActiveFilter] = useState('all'); // 'all', 'active', or 'inactive'

  const toggleDeathCauseForm = () => {
    setShowDeathCauseForm(!showDeathCauseForm);
    if (!showDeathCauseForm) {
      setEditingDeathCause(null); // Reset editing DeathCause when opening form for new DeathCause
    }
  };

  const handleEdit = (DeathCause) => {
    setEditingDeathCause(DeathCause);
    setShowDeathCauseForm(true);
  };

  const handleView = (DeathCause) => {
    setViewingDeathCause(DeathCause);
  };

  const handleSaveDeathCause = (DeathCauseData) => {
    if (editingDeathCause) {
      // Update existing DeathCause
      setDeathCauses(DeathCauses.map(DeathCause => 
        DeathCause.id === editingDeathCause.id ? { ...DeathCauseData, id: DeathCause.id } : DeathCause
      ));
    } else {
      // Add new DeathCause
      const newDeathCause = {
        ...DeathCauseData,
        id: DeathCauses.length > 0 ? Math.max(...DeathCauses.map(d => d.id)) + 1 : 1
      };
      setDeathCauses([...DeathCauses, newDeathCause]);
    }

    setShowDeathCauseForm(false);
    setEditingDeathCause(null);
  };
  ...
  // Filter DeathCauses based on search term and active filter
  const filteredDeathCauses = DeathCauses.filter(DeathCause => {
    // First apply status filter
    if (activeFilter === 'active' && !DeathCause.isActive) return false;
    if (activeFilter === 'inactive' && DeathCause.isActive) return false;
    
    // Then apply search term filter
    return (
      DeathCause.shortName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      DeathCause.longName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      DeathCause.printName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      DeathCause.description.toLowerCase().includes(searchTerm.toLowerCase())
    );
  });

  return (
    <div className="container mx-auto py-6 px-4">
      {/* Header section with search bar and Add DeathCause button */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold mb-4">Cause of Death Management</h1>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
          ...
          <div className="relative w-full md:w-1/2">
            ...
            <input 
              ...
              placeholder="Search DeathCauses..." 
              value={searchTerm}
              onChange={handleSearch}
            />
          </div>
          
          {/* Add DeathCause button */}
          <button
            onClick={toggleDeathCauseForm}
      ...
      <div>
        {showDeathCauseForm ? (
          <DeathCauseMasterForm 
            onClose={toggleDeathCauseForm} 
            onSave={handleSaveDeathCause} 
            initialData={editingDeathCause} 
          />
        ) : (
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 border-collapse">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredDeathCauses.length > 0 ? (
                    filteredDeathCauses.map((DeathCause) => (
                      <tr 
                        key={DeathCause.id} 
                        className={DeathCause.isActive ? "bg-[#D1FFBD]" : "bg-[#f3c2c2]"}
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          {DeathCause.shortName}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          {DeathCause.longName}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          {DeathCause.printName}
                        </td>
                    ...
                    <tr>
                      <td colSpan={6} className="px-6 py-4 text-center text-sm text-gray-500">
                        {searchTerm 
                          ? `No DeathCauses found matching "${searchTerm}"${activeFilter !== 'all' ? ` with '${activeFilter}' status` : ''}`
                          : `No ${activeFilter !== 'all' ? activeFilter : ''} DeathCauses found`
                        }
                      </td>
                    </tr>
                  )}
            ...
            <div className="p-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-900">{viewingDeathCause.shortName}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-orange-500">Long Name</p>
                  <p className="mt-1 text-sm text-gray-900">{viewingDeathCause.longName}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-orange-500">Print Name</p>
                  <p className="mt-1 text-sm text-gray-900">{viewingDeathCause.printName}</p>
                </div>
                <div className="col-span-2">
                  <p className="text-sm font-medium text-orange-500">Description</p>
                  <p className="mt-1 text-sm text-gray-900 max-h-16 overflow-y-auto">{viewingDeathCause.description}</p>
                </div>
              </div>
...
// DeathCause Master Form component with edit capabilities
const DeathCauseMasterForm = ({ onClose, onSave, initialData }) => {
    ...
    <div className="max-w-4xl mx-auto my-8 bg-white rounded-lg shadow-md">
      ...
      <div className="flex items-center p-4 bg-orange-500 text-white rounded-t-lg">
        ...
        <h1 className="text-2xl font-bold flex-grow text-center">
          {initialData ? 'UPDATE DeathCause' : 'ADD DeathCause'}
        </h1>
        <button 
          onClick={onClose}
      ...
      <div className="border-2 border-blue-500 rounded-lg m-6 p-6">
        {/* DeathCause Details Section */}
        <div className="relative mb-6 bg-white rounded-lg">
          <h2 className="text-lg font-semibold mb-6 px-4 py-2 bg-orange-500 text-white rounded">DeathCause DETAILS</h2>
          ...
          <div className="mb-6">
            ...
            <p className={`mt-1 text-sm ${formData.isActive ? "text-green-600" : "text-red-600"}`}>
              {formData.isActive ? "Y - DeathCause is active" : "N - DeathCause is inactive"}
            </p>
          </div>
