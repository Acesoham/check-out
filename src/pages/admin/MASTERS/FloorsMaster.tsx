...
// Sample Floors data for demonstration
const initialfloors = [
    {
      id: 1,
      isActive: true,
      ...
      printName: 'Ground Floor',
      description: 'The base level of a building, typically at street level'
    },
    {
      id: 2,
      isActive: true,
      shortName: 'G+1',
      longName: 'Ground Plus One',
      printName: '1 Floor',
      description: 'Ground floor plus one upper floor'
    },
    {
      id: 3,
      isActive: true,
      shortName: 'G+2',
      longName: 'Ground Plus Two',
      printName: '2 Floors',
      description: 'Ground floor plus two upper floors'
    },
    {
      id: 4,
      isActive: true,
      shortName: 'G+3',
      longName: 'Ground Plus Three',
      printName: '3 Floors',
      description: 'Ground floor plus three upper floors'
    }
  ];
  

// floors Management Dashboard for Zila Parishad / Gramin Vibhag
const Dashboard = () => {
  const [showFloorsForm, setShowFloorsForm] = useState(false);
  const [floors, setfloors] = useState(initialfloors);
  const [editingFloors, setEditingFloors] = useState(null);
  const [viewingFloors, setViewingFloors] = useState(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [activeFilter, setActiveFilter] = useState('all'); // 'all', 'active', or 'inactive'

  const toggleFloorsForm = () => {
    setShowFloorsForm(!showFloorsForm);
    if (!showFloorsForm) {
      setEditingFloors(null); // Reset editing Floors when opening form for new Floors
    }
  };

  const handleEdit = (Floors) => {
    setEditingFloors(Floors);
    setShowFloorsForm(true);
  };

  const handleView = (Floors) => {
    setViewingFloors(Floors);
  };

  const handleSaveFloors = (FloorsData) => {
    if (editingFloors) {
      // Update existing Floors
      setfloors(floors.map(Floors => 
        Floors.id === editingFloors.id ? { ...FloorsData, id: Floors.id } : Floors
      ));
    } else {
      // Add new Floors
      const newFloors = {
        ...FloorsData,
        id: floors.length > 0 ? Math.max(...floors.map(d => d.id)) + 1 : 1
      };
      setfloors([...floors, newFloors]);
    }

    setShowFloorsForm(false);
    setEditingFloors(null);
  };
  ...
  // Filter floors based on search term and active filter
  const filteredfloors = floors.filter(Floors => {
    // First apply status filter
    if (activeFilter === 'active' && !Floors.isActive) return false;
    if (activeFilter === 'inactive' && Floors.isActive) return false;
    
    // Then apply search term filter
    return (
      Floors.shortName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      Floors.longName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      Floors.printName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      Floors.description.toLowerCase().includes(searchTerm.toLowerCase())
    );
  });

  return (
    <div className="container mx-auto py-6 px-4">
      {/* Header section with search bar and Add Floors button */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold mb-4">Floors Management</h1>
      ...
      <div>
        {showFloorsForm ? (
          <FloorsMasterForm 
            onClose={toggleFloorsForm} 
            onSave={handleSaveFloors} 
            initialData={editingFloors} 
          />
        ) : (
              ...
              <table className="min-w-full divide-y divide-gray-200 border-collapse">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredfloors.length > 0 ? (
                    filteredfloors.map((Floors) => (
                      <tr 
                        key={Floors.id} 
                        className={Floors.isActive ? "bg-[#D1FFBD]" : "bg-[#f3c2c2]"}
                      >
                    ...
                    <tr>
                      <td colSpan={6} className="px-6 py-4 text-center text-sm text-gray-500">
                        {searchTerm 
                          ? `No floors found matching "${searchTerm}"${activeFilter !== 'all' ? ` with '${activeFilter}' status` : ''}`
                          : `No ${activeFilter !== 'all' ? activeFilter : ''} floors found`
                        }
                      </td>
                    </tr>
                  )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full">
            <div className="flex items-center justify-between p-4 bg-orange-500 text-white rounded-t-lg">
              <h2 className="text-xl font-bold">Floors Details</h2>
            ...
            <div className="p-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-900">{viewingFloors.shortName}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-orange-500">Long Name</p>
                  <p className="mt-1 text-sm text-gray-900">{viewingFloors.longName}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-orange-500">Print Name</p>
                  <p className="mt-1 text-sm text-gray-900">{viewingFloors.printName}</p>
                </div>
                <div className="col-span-2">
                  <p className="text-sm font-medium text-orange-500">Description</p>
                  <p className="mt-1 text-sm text-gray-900 max-h-16 overflow-y-auto">{viewingFloors.description}</p>
                </div>
              </div>
...
// Floors Master Form component with edit capabilities
const FloorsMasterForm = ({ onClose, onSave, initialData }) => {
    ...
    <div className="max-w-4xl mx-auto my-8 bg-white rounded-lg shadow-md">
      ...
      <div className="flex items-center p-4 bg-orange-500 text-white rounded-t-lg">
        ...
        <h1 className="text-2xl font-bold flex-grow text-center">
          {initialData ? 'UPDATE Floors' : 'ADD Floors'}
        </h1>
        <button 
          onClick={onClose}
      ...
      <div className="border-2 border-blue-500 rounded-lg m-6 p-6">
        ...
        <div className="relative mb-6 bg-white rounded-lg">
          <h2 className="text-lg font-semibold mb-6 px-4 py-2 bg-orange-500 text-white rounded">Floors DETAILS</h2>
          ...
          <div className="mb-6">
            ...
            <p className={`mt-1 text-sm ${formData.isActive ? "text-green-600" : "text-red-600"}`}>
              {formData.isActive ? "Y - Floors is active" : "N - Floors is inactive"}
            </p>
          </div>
