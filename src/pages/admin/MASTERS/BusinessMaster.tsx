...
// Businesss Management Dashboard for Zila Parishad / Gramin Vibhag
const Dashboard = () => {
  const [showBusinessForm, setShowBusinessForm] = useState(false);
  const [Businesss, setBusinesss] = useState(initialBusinesss);
  ...
  const toggleBusinessForm = () => {
    ...
    if (!showBusinessForm) {
      setEditingBusiness(null); // Reset editing Business when opening form for new Business
    }
  };
  ...
  const handleSaveBusiness = (BusinessData) => {
    if (editingBusiness) {
      ...
      setBusinesss(Businesss.map(Business => 
        Business.id === editingBusiness.id ? { ...BusinessData, id: Business.id } : Business
      ));
    } else {
      // Add new Business
      const newBusiness = {
        ...BusinessData,
        id: Businesss.length > 0 ? Math.max(...Businesss.map(d => d.id)) + 1 : 1
      };
      setBusinesss([...Businesss, newBusiness]);
    }
  ...
  // Filter Businesss based on search term and active filter
  const filteredBusinesss = Businesss.filter(Business => {
    // First apply status filter
    if (activeFilter === 'active' && !Business.isActive) return false;
    if (activeFilter === 'inactive' && Business.isActive) return false;
    
    // Then apply search term filter
    return (
      Business.shortName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      Business.longName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      Business.printName.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="container mx-auto py-6 px-4">
      {/* Header section with search bar and Add Business button */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold mb-4">Businesss Management</h1>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 border-collapse">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredBusinesss.length > 0 ? (
                    filteredBusinesss.map((Business) => (
                      <tr 
                        key={Business.id} 
                        className={Business.isActive ? "bg-[#D1FFBD]" : "bg-[#f3c2c2]"}
                      >
                    ...
                    <tr>
                      <td colSpan={6} className="px-6 py-4 text-center text-sm text-gray-500">
                        {searchTerm 
                          ? `No Businesss found matching "${searchTerm}"${activeFilter !== 'all' ? ` with '${activeFilter}' status` : ''}`
                          : `No ${activeFilter !== 'all' ? activeFilter : ''} Businesss found`
                        }
                      </td>
                    </tr>
                  )}
            ...
            <div className="p-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-900">{viewingBusiness.shortName}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-orange-500">Long Name</p>
                  <p className="mt-1 text-sm text-gray-900">{viewingBusiness.longName}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-orange-500">Print Name</p>
                  <p className="mt-1 text-sm text-gray-900">{viewingBusiness.printName}</p>
                </div>
                <div className="col-span-2">
                  <p className="text-sm font-medium text-orange-500">Description</p>
                  <p className="mt-1 text-sm text-gray-900 max-h-16 overflow-y-auto">{viewingBusiness.description}</p>
                </div>
              </div>
...
// Business Master Form component with edit capabilities
const BusinessMasterForm = ({ onClose, onSave, initialData }) => {
    ...
    <div className="max-w-4xl mx-auto my-8 bg-white rounded-lg shadow-md">
      ...
      <div className="border-2 border-blue-500 rounded-lg m-6 p-6">
        ...
        <div className="relative mb-6 bg-white rounded-lg">
          <h2 className="text-lg font-semibold mb-6 px-4 py-2 bg-orange-500 text-white rounded">Business DETAILS</h2>
          ...
          <div className="mb-6">
            ...
            <p className={`mt-1 text-sm ${formData.isActive ? "text-green-600" : "text-red-600"}`}>
              {formData.isActive ? "Y - Business is active" : "N - Business is inactive"}
            </p>
          </div>
